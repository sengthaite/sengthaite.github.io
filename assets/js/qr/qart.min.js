	    ...
	    value: function make(callback) {
	      ...
	      qrImage.onload = function () {
	        ...
	        coverImage.onload = function () {
	          ...
	          coverCanvas.getContext('2d', { willReadFrequently: true }).drawImage(coverImage, padding, padding, imageSize - 2 * padding, imageSize - 2 * padding);
	          var coverImageData = coverCanvas.getContext('2d', { willReadFrequently: true }).getImageData(0, 0, imageSize, imageSize);
	          var coverImageBinary = coverImageData.data;
	          var resultImageData = resultCanvas.getContext('2d', { willReadFrequently: true }).getImageData(0, 0, imageSize, imageSize);
	          ...
	          resultCanvas.getContext('2d', { willReadFrequently: true }).putImageData(resultImageData, 0, 0);
	          ...
	          for (var i = 0; i < patternPostion.length; i += 1) {
	            for (var j = 0; j < patternPostion.length; j += 1) {
	              ...
	              if (!(x === 6 && y === 50 || y === 6 && x === 50 || x === 6 && y === 6)) {
	                ...
	                var rectData = qrCanvas.getContext('2d', { willReadFrequently: true }).getImageData(rectX, rectY, rectWidth, rectHeight);
	                resultCanvas.getContext('2d', { willReadFrequently: true }).putImageData(rectData, rectX, rectY);
	              }
	            }
	          }
	          ...
	          if (typeof self.background !== 'undefined') {
	            scaledCanvas.getContext('2d', { willReadFrequently: true }).drawImage(bgCanvas, scaledPadding, scaledPadding, self.size - scaledPadding * 2, self.size - scaledPadding * 2);
	          }
	          scaledCanvas.getContext('2d', { willReadFrequently: true }).drawImage(coverImage, scaledPadding, scaledPadding, self.size - scaledPadding * 2, self.size - scaledPadding * 2);
	          scaledCanvas.getContext('2d', { willReadFrequently: true }).drawImage(resultCanvas, 0, 0, self.size, self.size);
	      ...
	      if (image) {
	        switch (fillType) {
	          ...
	          case 'scale_to_fit':
	            ...
	            canvas.getContext('2d', { willReadFrequently: true }).drawImage(image, x, y, newWidth, newHeight);
	            break;
	        }
	      }
	      return canvas;
